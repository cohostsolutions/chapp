...
// Sanitize chart ID to prevent injection attacks (only allow alphanumeric, dash, underscore)
const sanitizeChartId = (id: string): string => {
  return id.replace(/[^a-zA-Z0-9\-_]/g, '');
};

// Validate color value to prevent CSS injection (allows hex, rgb, hsl, and named colors)
const sanitizeColor = (color: string): string | null => {
  ...
  // Allow hsl/hsla
  if (/^hsla?\(\s*\d{1,3}(\.\d+)?\s*,?\s*\d{1,3}(\.\d+)?%?\s*,?\s*\d{1,3}(\.\d+)?%?\s*(,?\s*(0|1|0?\.\d+))?\s*\)$/.test(color)) {
    return color;
  }
 ...
 * 
 * SECURITY: Uses document.createElement for safer CSS injection instead of dangerouslySetInnerHTML.
 * All inputs are sanitized through sanitizeChartId, sanitizeColor, and sanitizeKey functions.
 */
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  React.useEffect(() => {
    ...
    // Generate CSS content using sanitized values only
      ...
      .map(([theme, prefix]) => {
        const variables = colorConfig
          .map(([key, itemConfig]) => {
            ...
            // Additional validation: ensure key and color are non-empty after sanitization
            if (!sanitizedKey || !color) return null;
            return `  --color-${sanitizedKey}: ${color};`;
          })
          .filter(Boolean)
          .join("\n");

        return variables ? `${prefix} [data-chart=${sanitizedId}] {\n${variables}\n}` : null;
      })
      .filter(Boolean)
      .join("\n");
    ...
    // Create style element safely using DOM API
