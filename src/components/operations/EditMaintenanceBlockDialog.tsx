...
import { MaintenanceBlock, CreateMaintenanceBlockInput } from '@/hooks/useMaintenanceBlocks';
...
interface EditMaintenanceBlockDialogProps {
  block: MaintenanceBlock | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (id: string, data: Partial<CreateMaintenanceBlockInput>) => Promise<boolean>;
  roomUnits: RoomUnit[];
  saving: boolean;
}
...
export function EditMaintenanceBlockDialog({
  block,
  open,
  onOpenChange,
  onSubmit,
  roomUnits,
  saving,
}: EditMaintenanceBlockDialogProps) {
  ...
  useEffect(() => {
    if (block && open) {
      setRoomUnitId(block.room_unit_id);
      setTitle(block.title || 'Maintenance');
      
      // Check if reason is a standard one or custom
      if (block.reason && MAINTENANCE_REASONS.includes(block.reason)) {
        setReason(block.reason);
        setCustomReason('');
      } else if (block.reason) {
        setReason('Other');
        setCustomReason(block.reason);
      } else {
        setReason('');
        setCustomReason('');
      }
      ...
      setEndDate(parseISO(block.end_date));
      setIsRecurring(block.is_recurring || false);
      setRecurrencePattern(block.recurrence_pattern || 'weekly');
    }
  }, [block, open]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await onSubmit(block.id, {
